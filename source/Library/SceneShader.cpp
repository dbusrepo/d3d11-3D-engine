...
namespace Library {
	...
	namespace BSPEngine {
		...
		void SceneShader::Initialize(ID3D11Device *device, HWND hwnd)
		{
		...
		void SceneShader::Apply(ID3D11DeviceContext* deviceContext)
		{
			if (mUseLighting) {
		...
		void SceneShader::InitializeShader(ID3D11Device *device, HWND hwnd)
		{
			HRESULT hr;
			...
			Utility::LoadBinaryFile(L"Content\\Shaders\\SceneLightVS.cso", compiledVS);
			...
			Utility::LoadBinaryFile(L"Content\\Shaders\\SceneLightPS.cso", compiledPS);
			...
			Utility::LoadBinaryFile(L"Content\\Shaders\\SceneNoLightVS.cso", compiledVS);
			// Create the vertex shader from the buffer.
			...
			Utility::LoadBinaryFile(L"Content\\Shaders\\SceneNoLightPS.cso", compiledPS);
			// Create the pixel shader from the buffer.
			...
			cbufferDescPS.ByteWidth = sizeof(glDataPShader);
			...
			// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
			hr = device->CreateBuffer(&cbufferDescPS, nullptr, &mCBufferPS);
			if (FAILED(hr))
			{
				throw D3DAppException("ID3D11Device::CreateBuffer() failed.", hr);
			}
		...
		void SceneShader::SetCBufferVS(ID3D11DeviceContext *deviceContext, XMMATRIX wvp)
		{
			HRESULT hr;
		...
		void SceneShader::SetCBufferPS(ID3D11DeviceContext * deviceContext, XMFLOAT3 worldCamPos, XMFLOAT2 clusterMapSize)
		{
		...
		void SceneShader::SetLightmapClusterMapsTextureArrayView(ID3D11DeviceContext * deviceContext, ID3D11ShaderResourceView * lightMapTexArrayView, ID3D11ShaderResourceView * clusterMapTexArrayView)
		{
			if (!mUseLighting) return;
		...
		void SceneShader::SetLeafDataView(ID3D11DeviceContext * deviceContext, ID3D11ShaderResourceView * leafDataView)
		{
			if (!mUseLighting) return;
			deviceContext->PSSetShaderResources(4, 1, &leafDataView);
		}

		void SceneShader::SetUseLighting(bool useLighting)
		{
			mUseLighting = useLighting;
		}

		void SceneShader::SetDiffuseAndNormalTextureArrayViews(ID3D11DeviceContext * deviceContext, ID3D11ShaderResourceView * textureArrView, ID3D11ShaderResourceView* textureArrNormalView)
		{
			// Set shader texture resource in the pixel shader.
			if (mUseLighting) {
