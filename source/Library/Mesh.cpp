...
namespace Library
{
	Mesh::Mesh(Model& model, const std::string& name)
		: mModel(model), mMaterial(nullptr), mName(name), mVertices(), mNormals(), mTangents(), mBiNormals(), mTextureCoordinates(), mVertexColors(), mFaceCount(0), mIndices()
	{
	}
	...
	void Mesh::CreateIndexBuffer(ID3D11Device* device, ID3D11Buffer** indexBuffer)
	{
		...
		indexBufferDesc.ByteWidth = sizeof(UINT) * (UINT)mIndices.size();
		...
		if (device->CreateBuffer(&indexBufferDesc, &indexSubResourceData, indexBuffer))
		{
