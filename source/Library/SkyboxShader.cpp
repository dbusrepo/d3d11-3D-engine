...
namespace Library {
	...
	void SkyboxShader::Initialize(ID3D11Device *device, HWND hwnd)
	{
		SetCurrentDirectory(Utility::ExecutableDirectory().c_str());

		InitializeShader(device, hwnd, L"Content\\Shaders\\SkyboxVS.cso", L"Content\\Shaders\\SkyboxPS.cso");
	}

	void SkyboxShader::Apply(ID3D11DeviceContext* deviceContext)
	{
	...
	void SkyboxShader::InitializeShader(ID3D11Device *device, HWND hwnd, const std::wstring &fileNameVS, const std::wstring &fileNamePS)
	{
		HRESULT hr;
	...
	void SkyboxShader::SetParameters(ID3D11DeviceContext *deviceContext, XMMATRIX wvp, ID3D11ShaderResourceView* textureView)
	{
		HRESULT hr;
